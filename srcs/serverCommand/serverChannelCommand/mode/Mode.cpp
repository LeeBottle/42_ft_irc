...
Mode::Mode(ClientManager &clients,
    ChannelManager &channels)
    : _checker(clients, channels), _parser(clients, channels), _applier()
{
}
...
bool    Mode::handle(Client &client,
...
void    Mode::broadcast(Client &client,
    Channel &channel, const ModeChange &change) const
{
    if (!change.changes.empty())
        CommandHelper::toAll(channel, ":" + client.prefix() + " MODE "
