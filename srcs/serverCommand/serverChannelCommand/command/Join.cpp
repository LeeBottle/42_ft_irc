...
Join::Join(ClientManager &clients,
...
bool    Join::handle(Client &client,
    const Parser &message)
{
    ...
    {
        CommandHelper::reply(client, ":ircserv 403 "
            + CommandHelper::target(client)
    ...
    if (result == JOIN_INVITE_ONLY)
        CommandHelper::reply(client, ":ircserv 473 " + CommandHelper::target(client)
            + " " + params[0] + " :Cannot join channel (+i)\r\n");
    else if (result == JOIN_BAD_KEY)
        CommandHelper::reply(client, ":ircserv 475 " + CommandHelper::target(client)
            + " " + params[0] + " :Cannot join channel (+k)\r\n");
    else if (result == JOIN_FULL)
        CommandHelper::reply(client, ":ircserv 471 " + CommandHelper::target(client)
            + " " + params[0] + " :Cannot join channel (+l)\r\n");
    else
    {
        join(*channel, client);
        CommandHelper::toAll(*channel, ":" + client.prefix() + " JOIN "
            + channel->name() + "\r\n");
...
Join::JoinResult
Join::checkPermission(Channel &channel,
    Client &client, const std::string &key) const
{
    if (channel.modes().inviteOnly()
        && !channel.invites().has(&client)
        && !channel.members().has(&client))
        return (JOIN_INVITE_ONLY);
    if (channel.modes().hasKey() && key != channel.modes().key()
        && !channel.members().has(&client))
        return (JOIN_BAD_KEY);
...
void    Join::join(Channel &channel,
    Client &client) const
{
