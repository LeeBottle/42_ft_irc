...
CommandMessage::CommandMessage(ChannelManager &channels,
...
void    CommandMessage::executePrivmsg(Client &client,
...
void    CommandMessage::sendChannelMessage(Client &client,
    const std::string &target, const std::string &text)
{
    Channel *channel;

    channel = _channels.findChannel(target);
    if (channel == NULL)
    {
