...
// 8. 채널 내 모든 구성원에게 KICK 상태 전달 및 epollout 쓰기 이벤트 활성화
	std::vector<int> members = channel.get_client_fds();
	for (size_t i = 0; i < members.size(); ++i)
	{
		int member_fd = members[i];
		
		// 당사자(Victim)에게는 방에서 강제로 쫓겨났음을 알리는 PART 패킷도 함께 전송 버퍼에 삽입
	...
	// 10. KICK 집행 이후 채널 멤버가 아무도 없다면 채널 리소스 소멸
	std::vector<int> remaining_members = channel.get_client_fds();
	if (remaining_members.empty() == true)
	{
...
/**
 * @brief INVITE 명령어를 처리하여 특정 유저를 채널에 초대합니다.
 */
